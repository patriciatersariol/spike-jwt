...
app.post("/login", (req, res) => {
  ...
  const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET) //retorna um objeto token
...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
    if (err) return res.sendStatus(403) //se der erro retorna token invalido ou ausente
    req.user = user //caso contrario  o objeto user vem preenchido com o payload do jwt informaçoes do usuario 
